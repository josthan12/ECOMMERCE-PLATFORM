...
export interface ReadyForCollectionProps {
  orderId: string;
  pickupAddress: string;
}

export default function ReadyForCollectionEmail({ orderId, pickupAddress }: ReadyForCollectionProps) {
  return (
    <Html>
      <Head />
      <Preview>Your {BRAND.storeName} order is ready for collection</Preview>
        ...
        <Container style={{ backgroundColor: '#FFFFFF', borderRadius: 8, overflow: 'hidden', maxWidth: 560 }}>
          ...
          <Section style={{ padding: '32px 32px 16px' }}>
            ...
            <Text style={{ color: BRAND.text, fontSize: 14, margin: '0 0 24px' }}>
              Your order is packed and ready — come by anytime to pick it up.
            </Text>
