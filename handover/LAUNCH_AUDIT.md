...
| USER REQUIRED | High | WCAG AA contrast remediation needs one follow-up deployment. | The initial deployment fixed all audited light-surface failures; the complete settled dark-theme scan and theme-aware chart SVG/legend checks passed. It also revealed four pre-existing accent usages on navy/ink sections now at `3.38:1`: the announcement divider, footer brand/location/link hover, and category-card count. The approved local correction changes only those usages to the existing light-gold token, which measures `10.62:1` on light ink and `12.03:1` on dark ink. Targeted lint, TypeScript, and the 43-page build pass. | Deploy the three-file follow-up and rerun the light-theme computed scan plus a dark-theme spot check. Then retain the owner-run screen-reader pass for Phase 3. |
