...
| USER REQUIRED | High | Browser security policies are implemented locally; deployment verification remains. | `next.config.ts` now applies an enforcing CSP, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, and a restrictive `Permissions-Policy` to all paths. The static CSP preserves ISR, limits executable/connect/frame origins to the app and documented Clerk/Cloudflare dependencies, blocks objects and framing, and omits development-only `unsafe-eval` from production. Targeted lint, TypeScript, Prisma generation, the 43-page production build, and generated route-manifest inspection pass. | Deploy, confirm all five headers on the custom domain, and smoke-test sign-in/sign-up, account, admin, cart, checkout, themes, product images, and browser console violations before marking PASS. Retest and adjust the Clerk origins when production Clerk credentials/domain replace the development instance. |
