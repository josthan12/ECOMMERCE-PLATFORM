...
**Final re-audit date:** 2026-08-04

**Decision:** **ENGINEERING GO WITH RECORDED MEDIUM/LOW RISKS. No High-severity engineering launch blocker remains open.**
...
- Production cleanup is deployed: removed
  `/api/admin/monitoring-test` returns `404`, while `/api/health` returns
  `200 {"status":"ok"}` with `Cache-Control: no-store, max-age=0`.
- The homepage, categories, cart, robots, sitemap, category detail, product
  detail, and search returned their expected production responses. HTTP
  redirects to HTTPS, signed-out account/admin/checkout routes redirect to
  sign-in, and the 26-URL sitemap parses as valid XML.
- The enforcing CSP, `X-Frame-Options`, `X-Content-Type-Options`,
  `Referrer-Policy`, `Permissions-Policy`, and HSTS remain deployed. The
  homepage was a Vercel cache hit.
- Whole-project ESLint, `tsc --noEmit`, Prisma schema validation, Prisma client
  generation, and the Next.js 16.2.11 production build passed. The build
  generated all 44 static pages and retained only the owner-controlled
  PostgreSQL future-`sslmode` warning.
- Static review reconfirmed Clerk plus database-role authorization on all 19
  admin API route files, owner-before-reconciliation enforcement on checkout
  success, signature checks on both webhooks, compare-and-set payment terminal
  transitions, durable email idempotency, and monitoring data minimization.
- No data, order, payment, email, dependency, application code, deployment,
  external-service configuration, secret, or server process changed during
  the re-audit.

This is an engineering launch-readiness decision, not final owner go-live
approval. Secret rotation, the minimal live payment, human keyboard/screen-
reader verification, real-catalogue approval, and any separately confirmed
database reset remain owner-controlled gates.

## Post-Audit Owner-Controlled Rotation Progress (2026-08-04)

- The owner confirmed that the Clerk production credentials and Sentry upload
  token were newly issued and never shared; they are treated as current.
- The Resend Production key was replaced and the site redeployed. Controlled
  newsletter `cmsefshpp000004l831obsax9` reported `Sent` with `1/1` delivery to
  the sole subscriber, and the owner confirmed inbox receipt. Revocation of the
  previous Resend key remains to be confirmed.
- `CRON_SECRET` was regenerated and replaced in Vercel Production and
  cron-job.org. The scheduler target was moved from the `vercel.app` hostname
  to the custom domain, and the owner-run test returned `200`. Separate
  read-only root and `/api/health` checks returned `200`. The scheduler still
  supplies the secret through `?secret=`, so the Medium finding remains open;
  confirm the five-minute job is enabled if it was paused during rotation.
- Neon Production was cut over to a separately created Console role and a new
  `DATABASE_URL` using `sslmode=verify-full`, while the original owner role was
  retained as rollback. The Vercel deployment is Ready; health, public
  catalogue, authenticated account/orders, and authenticated admin analytics
  passed. Sentry showed ten consecutive one-minute `200` checks with no ongoing
  issue, and Vercel showed zero warning/error/fatal logs in the observed
  30-minute window. The old owner credential remains valid until all other
  consumers are inventoried and its password can be safely reset.
- HitPay credential rotation is deferred until the owner has a live merchant
  account.
...
| PASS | High | The Next.js advisory blocker is remediated and deployed. | `next`, `eslint-config-next`, `@next/env`, the ESLint plugin, and the Windows SWC binary are `16.2.11`. The final re-audit's ESLint, TypeScript, Prisma validation/generation, and 44-page production build pass; the prior Next.js advisories remain absent, and production storefront/sitemap smoke checks passed. | Continue tracking the separately recorded residual advisories as their own dependency batch. |
...
| PASS | High | Production transactional order and newsletter sender configurations work. | The paid-order confirmation above sent successfully. Newsletter post `cmsbajgb1000004l810gl3dgn` sent to one subscribed customer with one delivery, one success, zero failures, and a Resend ID. After the Resend Production key rotation, controlled post `cmsefshpp000004l831obsax9` also reported `Sent` and `1/1`, and the owner confirmed receipt. The owner previously confirmed inbox receipt of the paid confirmation and both payment-failed messages. The older `$5` confirmation remains failed at its five-attempt cap as pre-fix test evidence. | Preserve valid unquoted sender syntax, confirm the previous Resend key is revoked, and continue monitoring delivery failures. |
...
| PASS | High | Production error tracking and uptime alerting are configured and verified. | On 2026-08-04 commit `6afb773` deployed production-only Sentry browser/Node/Edge/request/global-error capture, privacy filtering, release source maps, and a no-store `/api/health` database probe. The harmless admin-authenticated event mapped to the former test route with release/environment tags and without user identity, cookies, headers, body, or query data. Sentry Uptime monitor `7983784` recorded healthy `200` checks, a controlled 404 downtime issue, one owner-confirmed email alert, then a restored `200` recovery. The final re-audit confirmed the cleanup deployment: the test route now returns `404` and health remains `200`/no-store. The owner subsequently confirmed that the upload token is newly created and was never shared. | Retain the production-only Sentry variables, privacy hooks, exact CSP ingest origin, one-minute health monitor, three-failure/one-success thresholds, connected email alert, and current upload token. |

## Other Findings

| Status | Severity | Finding | Evidence | Recommended remediation |
| --- | --- | --- | --- | --- |
| FAIL | Medium | Cron secret can be supplied in a URL query parameter. | `app/api/cron/reconcile-orders/route.ts:17-18` accepts either Bearer authorization or `?secret=`. Query strings are commonly retained in request logs. The unauthenticated production route correctly returned `401`. Post-audit, the owner rotated `CRON_SECRET`, changed cron-job.org to the custom-domain endpoint, retained `?secret=`, and confirmed an owner-run scheduler test returned `200`. | After verifying the scheduler supports headers, remove query-string authentication and accept an exact Bearer token only. |
| FAIL | Medium | Authenticated checkout has no application-level abuse control. | `POST /api/checkout` requires a Clerk-backed user and atomically prevents overselling, but it has no rate limit, per-user pending-order ceiling, or equivalent abuse guard. A signed-in actor can repeatedly reserve stock until expiry, create database/payment-request pressure, and consume single-use promo codes under the owner-approved burn-on-order rule. No abusive production probe was performed. | Add a narrowly scoped checkout rate/abuse control in a separately reviewed batch; prefer a platform/WAF limit plus a server-enforced per-user pending-order policy that does not weaken payment idempotency. |
...
| FAIL | Medium | Runtime and transitive dependencies still require supported upstream remediation. | The locked graph is unchanged since the 2026-08-04 Sentry audit that reported 11 advisories (5 moderate, 6 high, no critical). Current primary-source review reconfirmed that Next.js 16.2.11 still resolves vulnerable `sharp@0.34.5` and its supported-upstream issue remains open; Prisma 7.8.0 remains its latest stable release. The exact package-manager count was not refreshed because the available execution runtime has no npm CLI; no alternate CLI was installed. | Recheck stable Next.js and Prisma releases, then perform a separately approved clean dependency update. Never run `npm audit fix --force`; rerun the exact npm audit when an approved npm-capable environment is available. |
...
| FAIL | Low | Clerk `user.created` processing is not retry-idempotent. | The signed webhook handler verifies Svix before mutation, but calls `prisma.user.create`. A legitimate duplicate delivery after the first success can hit the unique Clerk/email constraint and return a retriable server error even though the user already exists. The verified production signup succeeded; this is a retry-reliability gap, not an authentication bypass. | In a focused webhook-hardening change, make duplicate `user.created` delivery a successful no-op while preserving the rule that public signup can never assign `ADMIN`. |
...
| --- | --- | --- |
| PASS | Production build | The final re-audit's Prisma generation and Next.js 16.2.11 production build completed successfully and generated all 44 static pages. No development server was used. |
...
- [x] Re-ran the full technical audit on 2026-08-04 and issued an engineering
  GO with recorded Medium/Low risks and no open High-severity blocker.

No data wipe belongs to Phase 2.

## Phase 3 — Owner-Controlled Actions

- [x] Confirm the recently issued Clerk production credentials and Sentry
  upload token are current and were not shared.
- [x] Rotate Resend in Vercel Production, redeploy, and verify one controlled
  newsletter delivery. Confirm revocation of the previous key separately.
- [x] Rotate `CRON_SECRET`, move the cron-job.org target to the custom domain,
  and verify an owner-run `200` test. Confirm the schedule is enabled if it was
  paused; query-string removal remains a separate hardening change.
- [x] Cut Vercel Production over to a separate Neon Console role with
  `sslmode=verify-full` and verify public/authenticated database reads, Sentry
  health, and Vercel deployment/log state while retaining rollback.
- Inventory all remaining consumers, then reset—but do not delete—the original
  Neon owner role password. Defer HitPay/webhook rotation until a live merchant
  account exists. Complete any other remaining Vercel secret rotation without
  sharing values.
