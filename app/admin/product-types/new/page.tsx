...
import { AlertCircle, Tags, Plus, X } from 'lucide-react'
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4 rounded-lg border border-border-light bg-surface p-6 shadow-card">
          ...
          {fields.map((field, index) => (
            <div key={index} className="space-y-3 rounded-md border border-border-light bg-surface-muted p-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-text-muted">Field {index + 1}</span>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    value={field.label}
                    onChange={(e) =>
                      updateField(index, { label: e.target.value, key: generateKey(e.target.value) })
                    }
                ...
                <div>
                  <label className="mb-1 block text-xs text-text-muted">Key (auto-generated)</label>
                  <input
                    type="text"
                    value={field.key}
                    onChange={(e) => updateField(index, { key: e.target.value })}
                    placeholder="e.g. brand"
                    className="w-full rounded-md border border-border bg-surface-hover px-2.5 py-1.5 text-sm text-text focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent"
                  />
                </div>
              </div>
